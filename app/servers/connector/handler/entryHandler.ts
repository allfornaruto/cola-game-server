...
export class Handler {
    ...
    private onUserLeave(session: FrontendSession) {
        ...
        // 将该用户从游戏大厅中移除
        if (hallRid) this.app.rpc.game.gameRemote.kick.route(session, true)(uid, this.app.get("serverId"), hallRid);
        // 将该用户从房间内移除
        if (rid) this.app.rpc.game.gameRemote.kick.route(session, true)(uid, this.app.get("serverId"), rid);
        // 该用户是房主，游戏未开始则解散该房间
        if (!!ownRid) {
