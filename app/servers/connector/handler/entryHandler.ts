...
export class Handler {
  ...
  async enter(
    playerInfo: Cola.PlayerInitInfo,
    session: FrontendSession
  ): Promise<Cola.Response.ConnectorEnter> {
    const { uid, gameId, name } = playerInfo;
    const serverId = this.app.get("serverId");
    console.log(
      `[begin] connector.entryHandler.enter playerInfo=${JSON.stringify(
        playerInfo
      )}`
    );
    ...
    if (sessionPushResult)
      console.error(
        "connector.entryHandler.enter session.apushAll for session service failed! error is : %j",
        sessionPushResult.stack
      );
    ...
    console.log(
      `[end] connector.entryHandler.enter playerInfo=${JSON.stringify(
        playerInfo
      )}`
    );
    return {
  ...
  private onUserLeave(session: FrontendSession) {
    ...
    // 将该用户从游戏大厅中移除
    if (hallRid)
      this.app.rpc.game.gameRemote.kick.route(session, true)(
        uid,
        this.app.get("serverId"),
        hallRid
      );
    // 将该用户从房间内移除
    if (rid)
